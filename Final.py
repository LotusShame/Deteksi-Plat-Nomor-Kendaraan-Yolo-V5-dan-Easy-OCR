...
uploaded_image = st.file_uploader("Upload gambar pengendara sepeda motor (Minimal 1280x720 pixel)", type=["jpg", "jpeg", "png"])
...
if uploaded_image is not None:
    ...
    if image.size[0] >= 1280 and image.size[1] >= 720:
        ...
        if without_helmet_indices:
            ...
            for without_helmet_index in without_helmet_indices:
                ...
                if nearest_license_plate_index is not None:
                    ...
                    st.image(gray_plate, caption="Grayscale Plate", use_column_width=True)

                    # # Edge Detection
                    # st.write("### Edge Detection")
                    # edges = cv2.Canny(gray_plate, 50, 150, apertureSize=3)
                    # st.image(edges, caption="Edge Detected Plate", use_column_width=True)

                    # # Image Enhancement
                    # st.write("### Image Enhancement")
                    # contrast_factor = st.slider("Contrast Factor", key=f"contrast_{without_helmet_index}", min_value=1.0, max_value=3.0, value=1.5)
                    # brightness_factor = st.slider("Brightness Factor", key=f"brightness_{without_helmet_index}", min_value=0, max_value=100, value=30)
                    
                    # enhanced_plate = cv2.cvtColor(edges, cv2.COLOR_BGR2RGB)  # Use edges as the basis for enhancement
                    ...
                    # denoising_strength = st.slider("Denoising Strength", key=f"denoising_{without_helmet_index}", min_value=1, max_value=100, value=10)
                    ...
                    # erosion_kernel_size = st.slider("Erosion Kernel Size", key=f"erosion_{without_helmet_index}", min_value=1, max_value=20, value=5)
                    ...
                    plate_text = recognize_license_plate(gray_plate)
        ...
        st.write("Ukuran gambar harus minimal 1280x720 pixel.")
else:
    st.write("Silakan upload gambar pengendara sepeda motor (Minimal 1280x720 pixel).")
